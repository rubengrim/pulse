...
fn gi(@builtin(global_invocation_id) id: vec3<u32>) { 
    ...
    let z_linear = prepass_utils::prepass_depth(vec4f(vec2f(id.xy), 0.0, 0.0), 0u);
    var frag_coord = vec4(vec2f(id.xy), z_linear, 0.0);
    ...
    // color_out = normalize(abs(pbr_input.world_position.xyz));
...
fn sample_direct_light(p0: vec3f, n0: vec3f, rng_state: ptr<function, u32>) -> vec3f {
    ...
    // World space homogenous
    // let pl_h = mesh_instance.object_world * vec4f(pl_obj, 1.0);
    // let pl = pulse::utilities::transform_position(mesh_instance.object_world, pl_obj);
    let pl = vec3f(0.0, 2.0, 4.0);

    let to_light = pl - p0;
    // return normalize(abs(to_light));
    var shadow_ray = Ray();
    ...
    //     return vec3f(0.0);
    // }
    if pulse::utilities::are_aligned(p0, vec3f(p0.x, p0.y, 0.5), 0.01) {
